...
# class NeuralNetwork:
#     """
#     A wrapper for a bidirectional (possibly branched) neural network graph.

#     Parameters
#     ----------
#         input_layers: list[NeuralNetworkLayer]
#             List of input layers (roots) of the network. These are the layers without prev.

#     """
#     def __init__(self, input_layers: list[NeuralNetworkLayer]):
#         # input_layers: list of entry points (roots) to the graph
#         if not isinstance(input_layers, list):
#             input_layers = [input_layers]
#         self.input_layers = input_layers

#     def add_layer(self, new_layer: NeuralNetworkLayer, prev_layers: list[NeuralNetworkLayer]):
#         """
#         Add a new layer to the graph, connecting it to one or more previous layers (branching/merging).
#         """
#         if not isinstance(prev_layers, list):
#             prev_layers = [prev_layers]
#         for prev in prev_layers:
#             prev.add_next(new_layer)
#         new_layer.prev = prev_layers

#     def traverse(self, mode="forward"):
#         """
#         Generator to traverse the graph.
#         mode: "forward" (from inputs to outputs) or "backward" (from outputs to inputs)
#         """
#         visited = set()
#         stack = self.input_layers[:] if mode == "forward" else self.get_output_layers()
#         while stack:
#             layer = stack.pop()
...
#             next_layers = layer.get_next() if mode == "forward" else layer.get_prev()
#             if isinstance(next_layers, list):
#                 stack.extend(next_layers)
#             elif next_layers is not None:
#                 stack.append(next_layers)

#     def get_output_layers(self):
#         """
#         Return all layers with no next layers (output layers).
#         """
#         outputs = set()
#         visited = set()
#         stack = self.input_layers[:]
#         while stack:
#             layer = stack.pop()
#             if id(layer) in visited:
#                 continue
...
#             if not nexts or (isinstance(nexts, list) and len(nexts) == 0):
#                 outputs.add(layer)
#             else:
...
#     def __iter__(self):
#         # Default: forward traversal from input layers
#         return self.traverse(mode="forward")
